...
# show the first line with all properties for a single field and latest year
print(dt[id==1 & year==2022])


```


To simplify, one can also use a data.table as input. In that case use the function `bln_field_dt()`. The function checks whether all desired input variables are present. By default is the option to calculate the C saturation with RothC switched off. Again, only the indicators are calculated.
...
# calculate the BLN scoring values for the ecosystem services, without calculating C saturation with RothC
out <- bln_field_dt(dt.field, output = 'scores')

# calculate the BLN indicator and scoring values including RothC calculations
out <- bln_field_dt(dt.field, output = 'all', runrothc = TRUE)
